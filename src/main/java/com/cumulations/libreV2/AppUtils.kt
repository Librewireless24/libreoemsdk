...
object AppUtils {
    ...
    fun storeSSIDInfoToSharedPreferences(context: Context,deviceSSID: String, password: String) {
        context.getSharedPreferences("Your_Shared_Prefs", Context.MODE_PRIVATE)
            .apply {
...
//            return sceneObject?.playstatus == SceneObject.CURRENTLY_PLAYING
    ...
    fun isDMRPlayingFromOtherPhone(sceneObject: SceneObject?): Boolean {
        LibreLogger.d(TAG, "isDMRPlayingFromOtherPhone, ${sceneObject?.sceneName}")
        /* For Playing , If DMR is playing then we should give control for Play/Pause*/
        return sceneObject?.currentSource == Constants.DMR_SOURCE
                && sceneObject.playstatus == SceneObject.CURRENTLY_PLAYING
                && !sceneObject.playUrl.isNullOrEmpty()
                && !sceneObject.playUrl.contains(LibreApplication.LOCAL_IP)
    }

    fun isLocalDMRPlaying(sceneObject: SceneObject?): Boolean {
        /* For Playing , If DMR is playing then we should give control for Play/Pause*/
        val isLocalDMRPlaying = sceneObject?.currentSource == Constants.DMR_SOURCE
                && !sceneObject.playUrl.isNullOrEmpty()
                && sceneObject.playUrl?.contains(LibreApplication.LOCAL_IP)!!
                && (sceneObject.playstatus == SceneObject.CURRENTLY_PLAYING || sceneObject.playstatus == SceneObject.CURRENTLY_PAUSED)
        LibreLogger.d(TAG, "isLocalDMRPlaying, ${sceneObject?.sceneName} $isLocalDMRPlaying")
        return isLocalDMRPlaying
    }
   inline  fun <reified T : Serializable?> Intent.getSerializableIntent(intent: Intent, key:
   String):
           T {
    ...
    inline fun <reified T : Serializable> Bundle.serializableBundle(key: String): T? =
        when {
