    ...
    inner class ScanResultItemViewHolder(val itemBinding: CtListItemWifiBinding) :
        ...
        fun bindScanResultItem(scanResultItem: ScanResultItem?, position: Int) {
            ...
            if(scanResultItem!!.ssid==AppUtils.getConnectedSSID(context)){
                itemBinding.tvSsidName.setTextColor(ContextCompat.getColor(context, R.color.riva_app_text_color_enabled))
            }else{
                itemBinding.tvSsidName.setTextColor(ContextCompat.getColor(context, R.color.riva_text_color))
            }
