    ...
    fun stopScan() {
        ...
        if (mBleDevices.size == 0) {
            ...
            if (mBleDevices.size > 1) {
                binding.txtDeviceCount.text = buildString {
                    append(getString(R.string.we_have_found))
                    append(mBleDevices.size.toString())
                    append(getString(R.string.riva_devices_waiting))
                }
            } else {
                binding.txtDeviceCount.text = buildString {
                    append(getString(R.string.we_have_found))
                    append(mBleDevices.size)
                    append(getString(R.string.riva_device_waiting))
                }
            }
        }
    }
