...
//                artistName!!.text = message
                binding.tvAlbumName.text = ""
                binding.tvTrackName.text = ""
                LibreLogger.d(TAG, "we set the song name to empty for disabling " +
                        currentSceneObject!!
                .trackName
                + " in disabling view where artist name is " + message)
...
override fun messageRecieved(nettyData: NettyData) {
    LibreLogger.d(TAG, "Nowplaying: New message appeared for the device " + nettyData.getRemotedeviceIp())
    if (currentSceneObject != null && nettyData.getRemotedeviceIp().equals(currentIpAddress!!, ignoreCase = true)) {
        ...
        LibreLogger.d(TAG, "Packet is _" + packet.command)
        ...
        when (packet.command) {
            MIDCONST.MID_PLAYTIME.toInt() -> {
                ...
                MIDCONST.MID_CURRENT_PLAY_STATE.toInt() -> {
                    val message = String(packet.getpayload())
                    LibreLogger.d(TAG, "MB : 51, msg = $message")
                    try {
                        val duration = Integer.parseInt(message)
                ...
                MIDCONST.MID_PLAYCONTROL.toInt() -> {
                    ...
                    LibreLogger.d(TAG, "recieved 40 $message")
                    try {
                        when {
                ...
                MIDCONST.VOLUME_CONTROL -> {
                    ...
                    try {
                        ...
                        LibreLogger.d(TAG, "Recieved the current volume to be " +
                                currentSceneObject!!
                ...
                MIDCONST.SET_UI -> {
                    try {
                        ...
                        LibreLogger.d(TAG, "MB : 42, msg = $message")
    ...
    private fun setViews() {
        ...
        if (!currentSceneObject?.album_name.isNullOrEmpty()
                ...
                && !currentSceneObject?.album_name?.equals(binding.tvAlbumName.text?.toString())!!) {
    ...
    */
    private fun setTheSourceIconFromCurrentSceneObject() {
        ...
        when(currentSceneObject?.currentSource){
            ...
            AIRPLAY_SOURCE -> imgResId = R.drawable.airplay
            ...
            BT_SOURCE -> {
                ...
                when (currentSceneObject!!.playstatus) {
                    CURRENTLY_STOPPED -> disableViews(currentSceneObject!!.currentSource, getString(R.string.btOn))
    ...
    internal fun doSeek(pos: Int): Boolean {
        ...
        if (isLocalDMRPlayback) {
            ...
            val control = LUCIControl(currentSceneObject!!.ipAddress)
            LibreLogger.d(TAG, "Remote seek = " + pos + " total time of the song " +
                    currentSceneObject!!
            .totalTimeOfTheTrack)
            //duration = (pos * 0.01 * currentSceneObject.getTotalTimeOfTheTrack()) + "";
            duration = (pos * 1000).toString() + ""
            binding.seekBarSong.progress = pos
            LibreLogger.d(TAG, "Rempote Seek  = " + duration + " total time of the song " +
                    currentSceneObject!!
            .totalTimeOfTheTrack)
    ...
    override fun onActionFail(actionCallback: String?, response: UpnpResponse, cause: String?) {
        var cause = cause
        LibreLogger.d(TAG, " fragment that recieved the callback is " + currentSceneObject!!
            .ipAddress)
