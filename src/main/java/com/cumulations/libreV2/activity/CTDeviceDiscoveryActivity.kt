...
open class CTDeviceDiscoveryActivity : UpnpListenerActivity(), AudioRecordCallback,
   ...
   // lateinit var libreApplication: LibreEntryPoint
    ...
    private var busEventListener: Any = object : Any() {
        @Subscribe
        fun newDeviceFound(nodes: LSSDPNodes) {/*  LibreLogger.d(TAG_SECUREROOM, "newDeviceFound, node = " + nodes.friendlyname + " Ip Adress: " + nodes.ip + " cast model: " + nodes.castModel)*/
            insertDeviceIntoDb(nodes, "DI")
            //fetchUuidWithIp(nodes.ip)
        ...
        fun deviceGotRemoved(removedLibreDevice: RemovedLibreDevice?) {
            ...
            if (removedLibreDevice != null) {
                ...
                //    LibreLogger.d(TAG,TAG_DEVICE_REMOVED, "deviceGotRemoved before appInForeground ")
                if (appInForeground(this@CTDeviceDiscoveryActivity)) {
                    if (nodes != null) {/*  LibreLogger.d(TAG,TAG_DEVICE_REMOVED, "deviceGotRemoved, class name = " + this.javaClass.simpleName)*/
                        if (libreDeviceInteractionListner !is CTConnectingToMainNetwork) {/* LibreLogger.d(TAG,TAG_DEVICE_REMOVED, "deviceGotRemoved CTConnectingToMainNetwork")*/
                            alertDialogForDeviceNotAvailable(LSSDPNodeDB.getInstance().getTheNodeBasedOnTheIpAddress(removedLibreDevice.getmIpAddress()))
                        } else {/*  LibreLogger.d(TAG,TAG_DEVICE_REMOVED, "deviceGotRemoved Not " + "CTConnectingToMainNetwork")*/
                            removeTheDeviceFromRepo(removedLibreDevice.getmIpAddress())
                        }
                    }
                } else {
                    //LibreLogger.d(TAG,TAG_DEVICE_REMOVED, "deviceGotRemoved App Not InForeground ")
                    removeTheDeviceFromRepo(removedLibreDevice.getmIpAddress())
                }
            }
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //Vodafone code
       // LibreEntryPoint.getInstance().init(this)
        ...
        //  Log.d(TAG, "onCreate: getDeviceName: ${getDeviceName()}")OnePlus AC2001
    }

    private fun initUpnpProcessorListeners() {
        upnpProcessor = UpnpProcessorImpl(this)
        Log.d(APP_CLOSE, "addListener1 ")
    ...
    override fun onStart() {
        super.onStart()
        if (!isKeyStored) {
            LibreLogger.d(TAG_SECUREROOM,"onStart() called $isKeyStored")
            doSharedOperations()
        }
        initBusEvent()
//        if (appInForeground(this)) {
//            initDMRSearchService()
//        }
        Log.d(APP_CLOSE, "onStart: called")
    ...
    private fun initDMRForegroundService() {
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                Log.d(APP_FORGROUND, "initDMRForegroundService: if")
                ...
                Log.d(APP_FORGROUND, "initDMRForegroundService: else")
    ...
    private fun requestLocationPermission() {
        if (AppUtils.shouldShowPermissionRationale(this, Manifest.permission.ACCESS_FINE_LOCATION)) {
            ...
            Snackbar.make(parentView!!, R.string.permission_location_rationale, Snackbar.LENGTH_INDEFINITE).setAction(R.string.ok) { AppUtils.requestPermission(this@CTDeviceDiscoveryActivity, Manifest.permission.ACCESS_FINE_LOCATION, LOCATION_PERMISSION_REQUEST_CODE) }.show()
        } else {
            if (!sharedPreferenceHelper.isFirstTimeAskingPermission(Manifest.permission.ACCESS_FINE_LOCATION)) {
                ...
                showAlertDialog(getString(R.string.enableLocationPermit), getString(R.string.action_settings), getString(R.string.cancel), LOCATION_PERM_SETTINGS_REQUEST_CODE)
            }
    ...
    fun showSomethingWentWrongAlert(context: Context) {
        try {
            ...
            builder.setMessage(resources.getString(R.string.somethingWentWrong)).setCancelable(false).setPositiveButton("OK") { dialog, id -> alertDialog1!!.dismiss() }
    ...
    override fun onPause() {
        ...
        //Added By Shaik Have to ask SUMA to Review
        progressDialog.stop()
    }
    ...
    private fun setMusicPlayerListeners() {
        ...
        playPauseView?.setOnClickListener(object : View.OnClickListener {
            override fun onClick(view: View) {
                ...
                if (musicSceneObject.currentSource == NO_SOURCE || (musicSceneObject.currentSource == DMR_SOURCE && (musicSceneObject.playstatus == SceneObject.CURRENTLY_STOPPED || musicSceneObject.playstatus == SceneObject.CURRENTLY_NOTPLAYING))) {
            ...
            when (sceneObject.currentSource) {
                ...
                DMR_SOURCE, DMP_SOURCE, SPOTIFY_SOURCE -> {
                    if (sceneObject.currentSource == SPOTIFY_SOURCE) {
                        LibreLogger.d(TAG, "suma in spotify src" + currentAlbumArtView + currentAlbumnName + currentArtistName)
                        currentSourceView?.visibility = View.VISIBLE
                        currentSourceView?.setImageResource(R.mipmap.spotify)

//                        if(currentTrackName!="-1") {
//                            trackNameView?.text = currentTrackName
//                            trackNameView?.isSelected = true
//                        }
...
//            playPauseView?.isEnabled = false
    ...
    private fun playPauseNextPrevAllowed(currentSceneObject: SceneObject?): Boolean {
        ...
        return (currentSceneObject!!.currentSource != AUX_SOURCE/*&& currentSceneObject!!.currentSource != EXTERNAL_SOURCE*/ && currentSceneObject.currentSource != GCAST_SOURCE && currentSceneObject.currentSource != VTUNER_SOURCE && currentSceneObject.currentSource != TUNEIN_SOURCE && currentSceneObject.currentSource != NO_SOURCE && (currentSceneObject.currentSource != BT_SOURCE || (mNodeWeGotForControl.getgCastVerision() != null && mNodeWeGotForControl.bT_CONTROLLER != SceneObject.CURRENTLY_NOTPLAYING && mNodeWeGotForControl.bT_CONTROLLER != SceneObject.CURRENTLY_PLAYING) || (mNodeWeGotForControl.getgCastVerision() == null && mNodeWeGotForControl.bT_CONTROLLER >= SceneObject.CURRENTLY_PAUSED)))
    }
    ...
    fun handleAlexaViews(sceneObject: SceneObject) {
        ...
        // albumArtView?.visibility = View.GONE
        // playPauseView?.visibility = View.GONE
        LibreLogger.d(TAG, "suma in handle alexaplay icon" + sceneObject.currentSource)
        ...
        //  LibreLogger.d(TAG,"suma in handle alexaplay icon  alexa sceneobject\n"+"refresh token\n"+node
        //  .alexaRefreshToken+"trackName"+sceneObject.trackName+"album name"+sceneObject.album_name+"album name "+"refresh token null or empty\n"+node?.alexaRefreshToken.isNullOrEmpty()+sceneObject.playstatus+"play url"+sceneObject.playUrl)
        if (currentAlbumnName != null) {
            if (!currentAlbumnName.isEmpty()) {
                albumArtView?.visibility = View.VISIBLE
        ...
        if (sceneObject.playstatus == SceneObject.CURRENTLY_STOPPED || node?.alexaRefreshToken == "" || node?.alexaRefreshToken == null) {
            if (sceneObject.currentSource == SPOTIFY_SOURCE || sceneObject.currentSource == DMR_SOURCE || sceneObject.currentSource == DMP_SOURCE || sceneObject.currentSource == USB_SOURCE || sceneObject.currentSource == ALEXA_SOURCE) {
                ...
                if (node.getmDeviceCap().getmSource().isAlexaAvsSource) {
                    if (node?.alexaRefreshToken.isNullOrEmpty()) {
                        trackNameView?.text = getText(R.string.login_to_enable_cmds)
                        //  LibreLogger.d(TAG, "suma in handle alexaplay icon currently stopped2" + sceneObject
                        //  .playstatus)
                    } else {
                        trackNameView?.text = getText(R.string.speaker_ready_for_cmds)
                        // LibreLogger.d(TAG, "suma in handle alexaplay icon currently stopped3" + sceneObject
                        // .playstatus)
                    }
                } else {
                    trackNameView?.text = getText(R.string.speaker_ready_for_use)
                }
            }
            ...
            if (sceneObject.currentSource == BT_SOURCE || sceneObject.currentSource == AUX_SOURCE || sceneObject.currentSource == EXTERNAL_SOURCE || sceneObject.currentSource == NO_SOURCE) {
                ...
                // LibreLogger.d(TAG,"suma in handle alexaplay icon  alexa sceneobject\n"+sceneObject
                // .trackName+"album name"+sceneObject.album_name+"album name "+"refresh token null or empty\n"+node?.alexaRefreshToken.isNullOrEmpty()+sceneObject.playstatus+"play url"+sceneObject.playUrl)
                //   LibreLogger.d(TAG,"suma in handle alexaplay icon  alexa sceneobject
                //   LibreApplication\n"+"trackName"+LibreApplication.currentTrackName+"album name"+LibreApplication.currentAlbumnName+"album name "+"play url"+LibreApplication.currentArtistName)
                if (LibreApplication.currentTrackName != null) {
                    if (!LibreApplication.currentTrackName.isEmpty()) {
...
//                    LibreLogger.d(TAG, "MB : 42, msg = \$msg")
                    ...
                    if (cmdId == 3) {
                        ...
                        sceneObject.trackName = window.getString("Album")
        ...
        when (packet.command) {
            ...
            MIDCONST.FW_UPGRADE_INTERNET_LS9.toInt() -> {
                ...
                if (mNode.friendlyname.equals(sacDeviceNameSetFromTheApp, ignoreCase = true)) {
                    ...
                    when {
                        msg.equals(UPDATE_STARTED, ignoreCase = true) -> fwUpgradeData.updateMsg =
                            getString(R.string.mb223_update_started)
...
//            clearing data
            ...
            if(!mLuciThreadInitiated) {
                ...
                LibreLogger.d(TAG, "wifiConnected, restartAllSockets $restarted")
            }else{
                LibreLogger.d(TAG, "wifiConnected ELSE Condition")
            }
        }
    }

    private fun showNetworkChangeRestartAppAlert() {
        LibreLogger.d(TAG, "showNetworkChangeRestartAppAlert isNetworkChangesCallBackEnabled " + "=$isNetworkChangesCallBackEnabled")
        ...
        builder.setMessage(mMessage).setCancelable(false).setPositiveButton(getString(R.string.ok)) { dialog, id ->
            LibreLogger.d(TAG, "NetworkRestartAlert ok clicked")
            cleanUpCode(true)
            restartApp(applicationContext)
        }
        if (alertRestartApp == null || !alertRestartApp!!.isShowing) alertRestartApp = builder.show()
    }
    ...
    fun alertDialogForDeviceNotAvailable(mNode: LSSDPNodes?) {
        //  LibreLogger.d(TAG_DEVICE_REMOVED, "alertDialogForDeviceNotAvailable: ${mNode!!.friendlyname}")
        if (!this@CTDeviceDiscoveryActivity.isFinishing) {
            //   LibreLogger.d(TAG_DEVICE_REMOVED, "alertDialogForDeviceNotAvailable:After If")
            if (mNode == null) return
            //  LibreLogger.d(TAG_DEVICE_REMOVED, "alertDialogForDeviceNotAvailable: After Node Null Check ")
            alertDialog1 = null
            val alertDialogBuilder = AlertDialog.Builder(this@CTDeviceDiscoveryActivity)
            // set title
            ...
            alertDialogBuilder.setMessage(getString(R.string.removeDeviceMsg) + " " + mNode.friendlyname + " " + getString(R.string.removeDeviceMsg2)).setCancelable(false).setPositiveButton(getString(R.string.ok)) { dialog, id ->
                // if this button is clicked, close
                // current activity
                // LibreLogger.d(TAG_DEVICE_REMOVED, "alertDialogForDeviceNotAvailable: Clicked OK")
                alertDialog1!!.dismiss()
...
//            AlexaUtils.sendAlexaRefreshTokenRequest(devicIp)
    ...
    fun requestRecordPermission() {/*  LibreLogger.d(TAG,"==mandateDialog==18", "Record permission has NOT been granted. Requesting " + "permission.")*/
        if (AppUtils.shouldShowPermissionRationale(this, Manifest.permission.RECORD_AUDIO)) {
            ...
            } else {/*LibreLogger.d(TAG,"==mandateDialog==21", "Displaying Record permission Else showAlertDialog")*/
                //showAlertForRecordPermissionRequired()
                //   if (mandateDialog != null && mandateDialog!!.isShowing) mandateDialog!!.dismiss()
                showAlertDialog(getString(R.string.enableRecordPermit), getString(R.string.action_settings), getString(R.string.cancel), MICROPHONE_PERM_SETTINGS_REQUEST_CODE)
            }
    ...
    private fun showAlertDialog(message: String,
        ...
        if (mandateDialog == null) {
            ...
            builder.setPositiveButton(positiveButtonString) { dialogInterface, i ->
                mandateDialog!!.dismiss()
                customStartActivityForResult(requestCode, Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS).setData(Uri.fromParts("package", packageName, null)).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK).addFlags(Intent.FLAG_ACTIVITY_NO_HISTORY).addFlags(Intent.FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS))
            }
