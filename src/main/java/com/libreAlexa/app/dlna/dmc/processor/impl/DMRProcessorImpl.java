                    ...
                    m_controlPoint.execute(new GetVolume(m_renderingControl) {
                        ...
                        public void received(ActionInvocation actionInvocation, int currentVolume) {
                            if (m_currentVolume != currentVolume) {
                                ...
                                LibreLogger.d(TAG, "current volume" + currentVolume);
//                                m_checkGetVolumeInfo = false;
                            }
                        }
                    });
...
*/
    ...
    private Thread m_GetTransportStateThread = new Thread(new Runnable() {
        ...
        public void run() {
            ...
            while (m_isRunning) {
                if (m_hasSetURI && !m_isSettingURI) {
                    LibreLogger.d(TAG, "GetTransportStateThread is Running");
                    getControlPointStatus(counter);
                }
                try {
                    Thread.sleep(SLEEP);
                ...
                try {
                    ...
                    m_controlPoint.execute(new GetTransportInfo(m_avtransportService) {
                        ...
                        public void received(ActionInvocation invocation, TransportInfo transportInfo) {
                            //   LibreLogger.d(TAG, "TransportInfo State" + transportInfo.getCurrentTransportState());
                            LibreLogger.d(TAG, "Current Dmr State " + mCurrentDMRState);
                            ...
                            switch (transportInfo.getCurrentTransportState()) {
                                case PLAYING:
                                    ...
                                    fireOnPlayingEvent();
                                    LibreLogger.d(TAG, "Fireonplayingevent");

                                    break;
                                case PAUSED_PLAYBACK:
                                    fireOnPausedEvent();
                                    LibreLogger.d(TAG, "Fireonpauseevent");
                                    break;
                                case STOPPED:

                                    fireOnStopedEvent();
                                    LibreLogger.d(TAG, "Fireonstopevent" + "--mAvResponse--" + mCurrentDMRState +
...
/*
                                    ...
                                    if (currentSceneObject != null) {
                                        LibreLogger.d(TAG,"DMR_CURRENT_SOURCE", "" + currentSceneObject.getCurrentSource());
                                    }
...
//                                        String mIpaddress = m_device.getDetails().getBaseURL().getHost();
                                        LSSDPNodeDB mLssdpNodeDb = LSSDPNodeDB.getInstance();
                                        LSSDPNodes mNode = mLssdpNodeDb.getTheNodeBasedOnTheIpAddress(mIpaddress);
                                        LibreLogger.d(TAG, "Device State " + mNode.getDeviceState());
                                        if (mNode.getDeviceState().equals("M")) {
                                            if (currentSceneObject != null && currentSceneObject.getCurrentSource() == 2 ) {


                                                if (currentSceneObject.getPlayUrl()!=null && currentSceneObject.getPlayUrl().contains(com.libreAlexa.luci.Utils.getLocalV4Address(Utils.getActiveNetworkInterface()).getHostAddress())&& currentSceneObject.getTotalTimeOfTheTrack()-currentSceneObject.getCurrentPlaybackSeekPosition()<=2000) {
                                                    fireOnPlayCompletedEvent();
                                                    LibreLogger.d(TAG,"Prav-kannada", "Total position " + currentSceneObject.getTotalTimeOfTheTrack() + " -- Current " + currentSceneObject.getCurrentPlaybackSeekPosition());
                                                }
                    ...
                    LibreLogger.d(TAG, "TransportInfo State Failure Karuna" + "Interrupted Exception");
                    LibreLogger.d(TAG, "GetTransportStateThread is Running Interrupted");
                    e.printStackTrace();
                } catch (ExecutionException e) {
                    LibreLogger.d(TAG, "TransportInfo State Failure Karuna" + "ExecutionException Exception");
                    LibreLogger.d(TAG, "GetTransportStateThread is Running ExecutionException");
                    e.printStackTrace();
                } catch (TimeoutException e) {
                    LibreLogger.d(TAG, "TransportInfo State Failure Karuna" + "TimeoutException Exception" +
                            m_device.getDetails().getBaseURL().getHost());
                    LibreLogger.d(TAG, "GetTransportStateThread is Running TimeoutException");
...
//                        }
                    ...
                    LibreLogger.d(TAG, "TransportInfo State Failure Karuna" + "Exception Exception" +
                            m_device.getDetails().getBaseURL().getHost());
                    LibreLogger.d(TAG, "GetTransportStateThread is Running Exception");
                    /*mCountOfTransportInfoError ++ ;*/
                    /*if(mCountOfTransportInfoError>=5) */
                    {
...
//		});
    ...
    public void setURI(final String uri, final String uriMeta) {
      ...
      /*  m_hasSetURI = true;
        ...
        if (m_isSettingURI) {
            LibreLogger.d(TAG, "set AV uri pending:" + uri);
            m_hasPendingURI = true;
            return;
        }
        LibreLogger.d(TAG, "set AV uri now:" + uri);
        stop();
        ...
        m_controlPoint.execute(new SetAVTransportURI(m_avtransportService, uri, uriMeta) {
            ...
            public void success(ActionInvocation invocation) {
                ...
                LibreLogger.d(TAG,"Success setting URI to DMR");
           ...
           *//* @Override
            public ActionInvocation getActionInvocation() {
                ...
                LibreLogger.d(TAG,"actionException"+error);
            ...
            public void failure(ActionInvocation invocation, UpnpResponse response, String defaultMsg) {
                m_isSettingURI = false;
                LibreLogger.d(TAG,"Failed to SetAVTransportURI");
        ...
        LibreLogger.d(TAG, "Set Uri is  Called ");
        ...
        Stop stop = new Stop(m_avtransportService) {
            ...
            public void success(ActionInvocation invocation) {
                super.success(invocation);
                LibreLogger.d(TAG, "Stop is sucess");
                ...
                m_controlPoint.execute(new SetAVTransportURI(m_avtransportService, uri, uriMeta) {
                    ...
                    public void success(ActionInvocation invocation) {
                        ...
                        LibreLogger.d(TAG, "set uri is sucess");
                        m_isRunning = true;
                        m_isSettingURI = false;
                        iscompleted = false;
                    ...
                    @Override
                    public void failure(ActionInvocation invocation, UpnpResponse response, String defaultMsg) {
                        LibreLogger.d(TAG, "Set uri is failed");
                        m_isSettingURI = false;
            ...
            public void failure(ActionInvocation invocation, UpnpResponse response, String defaultMsg) {
                ...
                LibreLogger.d(TAG, "Stop is sucess in Failure ");
                mCurrentDMRState = DMR_STATE_STATUS.DEFAULT;
    ...
    public void play() {
        ...
        Play play = new Play(m_avtransportService) {
            ...
            public void failure(ActionInvocation invocation, UpnpResponse response, String defaultMsg) {
                ...
                mCurrentDMRState = DMR_STATE_STATUS.DEFAULT;
                LibreLogger.d(TAG, "Play is failed" + defaultMsg + "Response" + response.getResponseDetails());
                fireOnFailEvent("" + invocation.getAction(), response, defaultMsg);
    ...
    private void fireOnFailEvent(String action, UpnpResponse response, String message) {
        LibreLogger.d(TAG,"Praveen Seek action just failed now firing for all the subscribers");


        synchronized (m_listeners) {
            for (DMRProcessorListener listener : m_listeners) {
                LibreLogger.d(TAG,"Praveen Sending the listner callback to " +listener);
...
//        LibreLogger.d(TAG,TAG, "Call seek:" + position);
...
//            public void success(ActionInvocation invocation) {
//                super.success(invocation);
//                LibreLogger.d(TAG,TAG, "Seek success");
...
//
//                LibreLogger.d(TAG,TAG, "Seek Failed Praveena "+defaultMsg);
...
//    }
    ...
    @Override
    public void seek(String position) {
        LibreLogger.d(TAG, "Call seek:" + position);
        m_canUpdatePosition = false;
        ...
        Seek seek = new Seek(m_avtransportService, SeekMode.REL_TIME, position) {
            ...
            public void success(ActionInvocation invocation) {
                super.success(invocation);
                LibreLogger.d(TAG, "Seek success");
                m_canUpdatePosition = true; // changed for checking
            }
            ...
            public void failure(ActionInvocation invocation, UpnpResponse reponse, String defaultMsg) {
                ...
                LibreLogger.d(TAG, "Seek Failed Praveena "+defaultMsg);
                fireOnFailEvent(Constants.SEEK_FAILED + invocation.getAction(), reponse,
                        Seek.class.getSimpleName() + ":" + defaultMsg);
...
//                }
    ...
    public void setVolume(int newVolume) {
        LibreLogger.d(TAG, "Call setVolume");
        ...
        m_controlPoint.execute(new SetVolume(m_renderingControl, newVolume) {
            ...
            @SuppressWarnings("rawtypes")
            public void failure(ActionInvocation invocation, UpnpResponse operation, String defaultMsg) {
                LibreLogger.d(TAG, "setVolume fail: " + defaultMsg);
                /*fireOnFailEvent(invocation.getAction(), operation, defaultMsg);*/
            }
        });
    }
