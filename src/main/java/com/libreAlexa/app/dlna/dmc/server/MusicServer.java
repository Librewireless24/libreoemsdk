...
public class MusicServer {
  ...
  private String getCoverArtPath(Context context, long androidAlbumId) {
    ...
    try {
      ...
      LibreLogger.d(TAG, "Error while retriving album art for album id  " + androidAlbumId);
    }
    return "";
...
//		    buildOtherContainers(rootNode);
      prepareMediaServerBackground();
           ...
           LibreLogger.d(TAG, "Creating local device failed" + ex);
      mediaReady = false;
      return;
    }
  }
  ...
  private void getAudioContents(Cursor cursor) {
    ...
    if (cursor.moveToFirst()) {
      do {
        long mediaId = cursor.getInt(cursor.getColumnIndexOrThrow(MediaStore.Audio.Media._ID));
        ...
        Res res = new Res(
            new MimeType(mimeType.substring(0, mimeType.indexOf('/')), mimeType.substring(mimeType
                .indexOf('/') + 1)), size,
            "http://" + mediaServer.getAddressAndPort() + "/" + musicTrackItemId);
        ...
        musicTrack.addProperty(new DIDLObject.Property.UPNP.ALBUM_ART_URI(URI.create(
            "http://" + mediaServer.getAddressAndPort() + "/" + musicTrackItemId + "/album_art")));
        ...
        ContentTree.addNode(musicTrackItemId,
            new ContentNode(musicTrackItemId, musicTrack, filePath)
                .setAlbumArtpath(album_art_path));
        ...
        if (album != null) {
          ...
          if (albumsMap.containsKey(album)) {
            ...
            albumChild = new MusicAlbum(albumId,
                ContentTree.AUDIO_ALBUMS_ID, album, ContentTree.CREATOR, 0,
                new ArrayList<MusicTrack>());
...
//                String genres = getGenres(mediaId);
        ...
        LibreLogger.d(TAG,"getAudioContents genre = " + genres);
        if (genres != null) {
          Container genChild;
        ...
        LibreLogger.d(TAG,
            "added audio item, title:" + title + " ext:" + fileExtension + " mime:" + mimeType
                + " path:" + filePath);
  ...
  private int ordinalIndexOf(String str) {
    for (int i = 0; i < str.length(); i++) {
      if (str.charAt(i) == '/') {
//                LibreLogger.d(TAG,"AABBCCDD", "Position" + i);
        return i;
      }
    }
    return 0;
  ...
  private void buildFoldersContainers(String path, Container parentContainer, Item musicTrack,
      HashMap<String, Container> folderMap) {
    ...
    if (folderMap.containsKey(array[0])) {
      ...
      LibreLogger.d(TAG,"buildFolders"+ remainingPath);
    ...
    LibreLogger.d(TAG,"buildFolders We don't have any container for path " + path);
    ...
    for (int i = array.length - 2; i >= 0; i--) {
      ...
      folderChild = new MusicAlbum(folderID,
          ContentTree.AUDIO_FOLDER_ID, "" + array[i], ContentTree.CREATOR, 0,
          new ArrayList<MusicTrack>());
      folderChild.setWriteStatus(WriteStatus.NOT_WRITABLE);
      LibreLogger.d(TAG,"buildFolders created container for " + array[i]);
    ...
    LibreLogger.d(TAG,"buildFolders container for  " + previousContainer);
  }

//    @SuppressWarnings("deprecation")
//    private String getGenres(long mediaId) {
//        // TODO Auto-generated method stub
//        Uri uri = Uri.parse("content://media/exter/audio/media/" + mediaId + "/genres");
...
//            genre = c.getString(c.getColumnIndexOrThrow(MediaStore.Audio.GenresColumns.NAME));
...
/*
  ...
  private void prepareMediaServerBackground() {
    ...
    Cursor cursor = context.getContentResolver()
        .query(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, AUDIO_PROJECTION, null, null,
