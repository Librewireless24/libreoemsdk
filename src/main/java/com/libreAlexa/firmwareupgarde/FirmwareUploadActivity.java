...
import com.cumulations.libreV2.CustomProgressDialog;
...
public class FirmwareUploadActivity extends Activity {
    ...
    private final static String TAG = FirmwareUploadActivity.class.getSimpleName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!checkIfInAssets("lsimage")) {
            ...
            LibreLogger.d(TAG, "Device ip " + device_ip + " device name = " + device_name);
    ...
    private void copyFileOrDir(String path) {
        ...
        String assets[] = null;
        try {
            LibreLogger.d(TAG,"tag copyFileOrDir() " + path);
            assets = assetManager.list(path);
            if (assets.length == 0) {
                copyFile(path);
            } else {
                String fullPath = TARGET_BASE_PATH + path;
                LibreLogger.d(TAG,"tag path=" + fullPath);
                File dir = new File(fullPath);
                if (!dir.exists() && !path.startsWith("images") && !path.startsWith("sounds") && !path.startsWith("webkit"))
                    if (!dir.mkdirs())
                        LibreLogger.d(TAG,"tag could not create dir " + fullPath);
            ...
            LibreLogger.d(TAG,"tag I/O Exception "+ex);
        }
    }
    ...
    private void copyFile(String filename) {
        ...
        try {
            LibreLogger.d(TAG,"tag copyFile() " + filename);
            ...
            LibreLogger.d(TAG,"tag Exception in copyFile() of " + newFileName);
            LibreLogger.d(TAG,"tag Exception in copyFile() " + e.toString());
        }
